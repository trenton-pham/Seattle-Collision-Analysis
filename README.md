...
| `model.ipynb` | Modeling: creating categorical predictors (risk label), training a decision tree (experimenting hyperparameter tuning with GridSearchCV and RandomizedSearchCV) on label-derived features |
...
- Decision tree classifier: A decision tree tree is trained on a feature set that describes zone context: spatial cells (`cell_lat`, `cell_lon`), temporal ratios (`peak_hour`, `night_ratio`, `weekend_ratio`, `winter_ratio`, `summer_ratio`), collision mix (`avg_pedestrian_count`), and road structure (`intersection_ratio`, parsed from the `LOCATION` string).
...
- The decision tree classifies grid cells into three risk tiers with **70.2% 5-fold CV accuracy** and **69.5% test accuracy**.
- ~~Class-level performance is asymmetric: **Low Risk** zones are identified most reliably (F1 = 0.77), while **High Risk** zones show **high precision (0.84) but low recall (0.51)**. When the model flags a zone as high-risk it is usually correct, but it misses roughly half of the true high-risk zones, which default into the Medium tier.~~ After implementing hyperparameter tuning, the class performance level highly improved. Specifically, the recall score improved considerably in the recall in the **High Risk** zone **(0.51 -> 0.67 (GridSearchCV))**.
...
- When modeling, an equal qcut into thirds is used when labeling risk values for each zone (low, medium, high); however, this isn't representative of the true severity of the collision, and a new refined approach on labeling risk values will be worked on soon.
...
- Building a supervised classification model with hyperparameter tuning to predict collision risk levels per zone.
